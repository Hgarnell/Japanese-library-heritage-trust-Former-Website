   <!-- google fonts -->
    <link href="https://fonts.googleapis.com/css?family=M+PLUS+Rounded+1c:900%7cSawarabi+Gothic&display=swap" rel="stylesheet">
    
</head>
<body>
 
    <div class="wrapper">
            
        <nav class="box">
                <a href="jp_index.php"> <img src="image/logo.png"  class="center" alt="JKET logo" style="width:120px"/> </a>
                <h1> JKET </h1>
                <p>Japan Kauri Education Trust</p>
                
                <!-- side menu start https://www.w3schools.com/howto/howto_js_dropdown_sidenav.asp -->
                <div class="sidenav">
                        <a class="dropdown-btn">
                            <div class="menu">
                            <div class="bar1"></div>
                            <div class="bar2"></div>
                            <div class="bar3"></div>
                            </div>
                            メニュー
                        </a>
                            <div class="dropdown-container"> <!-- start of menu dropdown-->
                                <a href = "jp_index.php"> ホームページ </a>
                                <a href="jp_heritagecentre.php">日本語継承センター</a>
                                    <!-- Dropdown container start-->
                                <button class="dropdown-btn">Japanese Childrens <br> Library</button> 
                                    <div class="dropdown-container">
                                        <a href="jp_library.php">情報</a>
                                       <!-- <a href="jp_search.php">本探し</a>-->
                                    </div>
                                <a href="jp_parentalsupport.php">育児支援</a>
                                <a href="jp_contact.php">連絡先</a>
                                <a href="jp_donation.php">ドネーション</a>
                                <a href="jp_annual_report.php">年次報告書 </a>

                            </div> <!-- end of menu drop down -->

                </div> <!-- end of sidenav -->
        </nav> <!-- end navigation-->

        <?php include'jp_mobileheader.php'?>